...
| Kept from regressing, unattended | **19 checks** — [ui-smoke.mjs](../scripts/ui-smoke.mjs), 11 of 14 areas, ~16 s |
...
- **`scripts/*-test.mjs`** — 29 harnesses that speak the engine's WS/HTTP protocol
...
- **`node scripts/ui-smoke.mjs`** — the unattended browser suite: one slot, every check
  in order, non-zero exit on failure. ~16 s for the current 19 checks, most of which
