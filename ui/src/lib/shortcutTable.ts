...
export const PANE_HINTS: Record<QuickHelpPane, { title: string; items: ShortcutBinding[] }> = {
  timeline: {
    ...
    items: [
      ...
      { keys: ["↑", "↓"], what: "Move track selection · Shift extends the range" },
...
export const SHORTCUT_GROUPS: ShortcutGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { keys: ["0"], what: "Stop" },
      { keys: ["*"], what: "Exclusively arm selected track(s), then record" },
  ...
  },
  {
    ...
    items: [
      ...
      { keys: ["Ctrl+Alt+M"], what: "Cycle the UI shell — Classic / Ribbon / Workspaces" },
      { keys: ["F3 / Shift+F3"], what: "Toggle Mixer (mode selected in Settings → General)" },
      { keys: ["F11 / Shift+F11"], what: "Open Instrument Rack (mode selected in Settings → General)" },
      { keys: ["Alt+1…9"], what: "Switch workspaces (Workspaces shell)" },
      { keys: ["Alt+T"], what: "Production Techniques wizard" },
